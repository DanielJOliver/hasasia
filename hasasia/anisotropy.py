...
class Anisotropy(GWBSensitivityCurve):
    ...
    @property
    def S_eff_fkkprime(self):
        """
        Strain power sensitivity.
        """
        if not hasattr(self, '_S_eff_fkkprime'):
            self._S_eff_fkkprime = 1.0 / np.sum(self.S_SkyI, axis=1)
        return self._S_eff_fkkprime
    ...
    def h_c_aniso_pixel(self, freq_idx):
        """Characteristic Strain Sensitivity at a Specific Frequency for Each Pixel"""
        return np.sqrt(self.freqs[freq_idx] * self.S_eff_aniso_pixel(freq_idx))  # Shape: (768,)
